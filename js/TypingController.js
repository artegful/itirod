...
export class TypingController {
  ...
  HandleBackspace () {
    ...
    const span = this._spans[this._typedCharacters]
    span.className = ''
  ...
  HandleNextCharacter (key) {
    ...
    const span = this._spans[this._typedCharacters]
    const isCorrect = span.innerText === key
  ...
  Finish () {
    const gameFinished = new CustomEvent('gameFinished', {
      bubbles: true,
      cancelable: false,
      detail: {
  ...
  StartCountdown () {
    ...
    if (this._mode === modes.time) {
      this._countdown = setInterval(() => {
        ...
        const interval = Math.floor((now - this._startTime) / 1000)
        const remainingTime = this._totalTime - interval
